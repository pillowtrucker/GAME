...
void parse_layers(tson::Layer &l, gq::ActualLayerType lt,
  ...
  for (auto &tileobj : l.getObjects()) {
    ...
    String tilename =
        Unicode::FromUTF8(tileset.getTile(tileobj.getGid())->getType());
    ...
    if (!(tile_descriptions.find(unique_tile_name) ==
          tile_descriptions.end())) {
      tiledesc.emplace(tile_descriptions[unique_tile_name]);
    }
    registry.emplace<gq::Object>(entity,
                                 gq::Object{.unique_gid = tileobj.getGid(),
                                            .name = tilename,
                                            .descr = tiledesc});
